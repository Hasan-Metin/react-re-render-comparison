  ...
  const patterns: Omit<PatternCardProps, "onNavigate">[] = [
    {
      ...
      description:
        "Most isolated render behavior. Each component manages its own state and only re-renders when its own state changes. Siblings and parent are never affected.",
      renderBehavior:
        "Isolated re-renders. When a child component increments its count, only that child re-renders. Parent and sibling components remain untouched. This pattern provides maximum component isolation.",
      pros: [
        "Minimal re-renders - only affected component updates",
        "memo() works effectively - no prop changes",
        "Siblings never re-render on state changes",
        "Parent doesn't re-render when children update",
        "Good scalability for independent components",
      ],
      cons: [
        "Refs add complexity for cross-component communication",
        "Parent cannot easily read children's current state",
        "Goes against React's declarative data flow philosophy",
        "State scattered across components - harder to debug",
        "useImperativeHandle is meant as an escape hatch, not primary pattern",
      ],
    },
    {
    ...
    },
    {
      ...
      description:
        "Demonstrates a common misuse of Context for frequent state updates. Context is designed for dependency injection (themes, auth, locale), not as a state manager for rapidly changing values.",
      renderBehavior:
        "Every context consumer re-renders on any context change. This demo shows what happens when Context is misused for frequent updates - a pattern I've seen many developers fall into.",
      pros: [
        "No prop drilling - values available anywhere in tree",
        "Clean component interfaces",
        "Perfect for themes, auth, locale (infrequent changes)",
        "React recommends Context + useReducer for scaling up",
      ],
      cons: [
        "This demo shows MISUSE - frequent updates in single context",
        "ALL consumers re-render on ANY context change",
        "memo() doesn't help - context bypasses it",
        "Should split contexts or use proper state management for frequent updates",
      ],
    },
  ];
  ...
  return (
    <div className="overview">
      <header className="overview-header">
        <h1>React Re-render Behavior: Under the Hood</h1>
        <p>
          Explore how different state patterns affect component re-renders.
          This is what I've learned from building enterprise React apps - each pattern has trade-offs depending on your use case.
        </p>
      </header>
      ...
      <div className="comparison-table">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td>Re-render Isolation</td>
              <td>Most isolated</td>
              <td>Parent + changed children</td>
              <td>All consumers (when misused)</td>
            </tr>
            <tr>
